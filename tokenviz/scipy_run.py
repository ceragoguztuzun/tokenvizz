...
def process_on_gpu(rank, world_size, model_name, available_gpus, max_chunks=None):
    setup_logging(rank)
    ...
    dataset = DNADataset("/home/cxo147/ceRAG_viz/data/hg38.fa", rank, world_size, chunk_size, max_chunks=max_chunks)
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"GPU {rank} - Processing chunks", position=rank)):
        ...
        try:
            ...
            # Process the batch
            edges, weights, node_info, unique_nodes = create_graph(all_token_positions, attention_weights_per_sample, threshold=0.01)
...
def main():
    TEST_MODE = True  # Set to True for testing with smaller data
    MAX_CHUNKS = 10    # Number of chunks to process for testing
    ...
    # Pass MAX_CHUNKS only if TEST_MODE is True
    if TEST_MODE:
        mp.spawn(
            process_on_gpu, 
            args=(world_size, model_name, available_gpus, MAX_CHUNKS), 
            nprocs=world_size, 
            join=True
        )
    else:
        mp.spawn(
            process_on_gpu, 
            args=(world_size, model_name, available_gpus, None), 
            nprocs=world_size, 
            join=True
        )
    ...
    if mapped_edges:
        ...
        saved_matrix_fn = "attention_graph_nothreshold.npz"
    ...
    if node_id_to_index:
        ...
        logging.info("Main: Combined node information saved to node_info.json")
    else:
